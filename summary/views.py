...
def summary_list(request):
    ...
    return render(request, 'summary/summary_list.html', {'summaries': summaries})

def summary_detail(request, pk):
    summary = get_object_or_404(Summary, pk=pk)
    return render(request, 'summary/summary_detail.html', {'summary': summary})

def pdf_view(request):
    url = '../summary/pdf/reflect.pdf'
...
def subject(request, pk):
    ...
    return render(request, 'subject.html', {'summaries': summaries, 'subject': subject })

def uploadView(request):
    if request.method == 'POST' and request.FILES['myfile']:
        myfile = request.FILES['myfile']
        fs = FileSystemStorage()
        filepath = '/summary/pdf/%s' % myfile.name #change this to summary pk
...
class addSummary(TemplateView):
    ...
    def post(self, request):
         ...
         if request.method == 'POST' and request.FILES['myfile']:
             myfile = request.FILES['myfile']
             fs = FileSystemStorage()
             summaryfile = '../pdf/%s' % myfile.name
             filename = fs.save(summaryfile, myfile)

         summary = Summary(title=title, user=user, book=book, subject=subject, university=university, filepath=summaryfile, rating=0, numRates=0)
         summary.save()
...
class logout_auth(TemplateView):
    ...
    def get(self, request):
        ...
        return render(request, self.template_name, { 'message' : 'You have logged out'})
